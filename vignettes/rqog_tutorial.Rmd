...
basic %>% 
  count(ccp_cc) %>% 
  mutate(ccp_cc_lab = label_data(data = ., variable.data = "ccp_cc", metadata = meta_basic_ts_2020))
```
...
plot_d <- basic_cs %>% 
  ...
  ungroup() %>% 
  # label
  mutate(ht_region_lab = label_data(data = ., variable.data = "ht_region", metadata = meta_basic_cs_2020),
         ht_regtype_lab = label_data(data = ., variable.data = "ht_regtype", metadata = meta_basic_cs_2020)) %>% 
  na.omit()
head(plot_d)
```
