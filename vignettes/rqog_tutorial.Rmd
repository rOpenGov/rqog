...
```{r}
library(rqog)
meta_basic_ts[grepl("Corruption", meta_basic_ts$name, ignore.case = TRUE),]
```
...
```{r}
meta_basic_ts %>% filter(code == "ccp_cc") %>% select(value,label)
```
...
basic %>% 
  count(ccp_cc) %>% 
  mutate(ccp_cc_lab = meta_basic_ts[meta_basic_ts$code == "ccp_cc",]$label[match(ccp_cc,meta_basic_ts[meta_basic_ts$code == "ccp_cc",]$value)])
```
...
```{r}
meta_basic_cs %>% filter(code %in% c("ht_region","ht_regtype")) %>% 
...
plot_d <- basic_cs %>% 
  ...
  ungroup() %>% 
  # label
  mutate(ht_region_lab = label_data(data = ., variable.data = "ht_region", metadata = meta_basic_cs),
         ht_regtype_lab = label_data(data = ., variable.data = "ht_regtype", metadata = meta_basic_cs)) %>% 
  na.omit()
head(plot_d)
```
...
ind_name <- unique(meta_basic_cs[meta_basic_cs$code == "ht_regtype",]$name)
group_name <- unique(meta_basic_cs[meta_basic_cs$code == "ht_region",]$name)
