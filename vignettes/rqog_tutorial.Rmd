...
Packages is shipped with six metadatas for each year (2016-2019) `meta_basic_cs_2019`, `meta_basic_ts_2019`, `meta_std_cs_2019`, `meta_std_ts_2019`, `meta_oecd_cs_2019` and  `meta_oecd_ts_2019`. Data frames are generated from original `spss` versions of data using `tidymetadata::create_metadata()`-function.  
...
```{r}
library(rqog)
meta_basic_ts_2019[grepl("Corruption", meta_basic_ts_2019$name, ignore.case = TRUE),]
```
...
```{r}
meta_basic_ts_2019 %>% filter(code == "ccp_cc") %>% select(value,label)
```
...
basic %>% 
  count(ccp_cc) %>% 
  mutate(ccp_cc_lab = meta_basic_ts_2019[meta_basic_ts_2019$code == "ccp_cc",]$label[match(ccp_cc,meta_basic_ts_2019[meta_basic_ts_2019$code == "ccp_cc",]$value)])
```
...
basic %>% 
  count(ccp_cc) %>% 
  mutate(ccp_cc_lab = label_data(data = ., variable.data = "ccp_cc", metadata = meta_basic_ts_2019))
```
...
plot_d <- basic_cs %>% 
  ...
  ungroup() %>% 
  # label
  mutate(ht_region_lab = label_data(data = ., variable.data = "ht_region", metadata = meta_basic_cs_2019),
         ht_regtype_lab = label_data(data = ., variable.data = "ht_regtype", metadata = meta_basic_cs_2019)) %>% 
  na.omit()
head(plot_d)
```
...
ind_name <- unique(meta_basic_cs_2019[meta_basic_cs_2019$code == "ht_regtype",]$name)
group_name <- unique(meta_basic_cs_2019[meta_basic_cs_2019$code == "ht_region",]$name)

ggplot(plot_d, aes(x=ht_regtype_lab,y=n)) + 
  geom_col() + 
  facet_wrap(~ht_region_lab, scales = "free") + 
  theme_ipsum_ps() + theme(axis.text.x = element_text(angle = 90, size = 7)) +
