...
read_qog <- function(which.data = "basic", data.type="time-series", data.dir = "~/tmp") {
        ...
        if (data.type == "time-series") {
            ...
            if (which.data == "basic")          name.csv <- "qog_bas_ts_jan15.csv"
            if (which.data == "standard")       name.csv <- "qog_std_ts_jan15.csv"
            if (which.data == "oecd")           name.csv <- "qog_oecd_ts_jan15.csv"        
        }
        
        # time-series data
        if (data.type == "cross-sectional") {
            # local csv that will be read
            if (which.data == "basic")          name.csv <- "qog_bas_cs_jan15.csv"
            if (which.data == "standard")       name.csv <- "qog_std_cs_jan15.csv"
            if (which.data == "oecd")           name.csv <- "qog_oecd_cs_jan15.csv"
        }
